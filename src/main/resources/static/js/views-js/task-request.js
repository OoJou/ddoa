$(function () {
    get_task_of_user_create(1);
});

//请求get_task_of_user_create接口数据
function get_task_of_user_create(pageNum) {
    ...
    $.ajax({
        url:"/task/get_task_of_user_create.do",
        type:"POST",
        dataType:"json",
        data:{
...
function set_task_html(result) {
    ...
    $.each(taskList,function (index, item) {
        ...
        taskTitleTd.click(function () {
            window.location.href="task_request_details?id="+item.taskId+"&html=task_request";
        });
...
function set_pager(pageList) {
    ...
    if(pageList.hasPreviousPage==true){
        ...
        prePageLi.click(function () {
            get_task_of_user_create(pageList.prePage);
        });
    ...
    if(pageList.hasNextPage==true) {
        ...
        lastPageLi.click(function () {
            get_task_of_user_create(pageList.lastPage+1);
        });
        ...
        nextPageLi.click(function () {
            get_task_of_user_create(pageList.nextPage);
        });
...
$(document).on("click",".pagination-btn",function () {
    get_task_of_user_create($(this).text());
});
//点击跳转
$(document).on("click",".jump-pager-button",function () {
    get_task_of_user_create($(".jump-pager-input").val());
});
