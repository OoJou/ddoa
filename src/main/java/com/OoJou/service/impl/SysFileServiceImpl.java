...
public class SysFileServiceImpl implements ISysFileService {
	...
	@Override
	public ServerResponse<SysFile> downloadFile(HttpServletRequest request, HttpServletResponse response,
			String downloadath, int fileId) {// 前端点击确认下载，只传入id。根据id查出文件名和路径，传入工具类开始下载
	...
	@Override
	public ServerResponse<String> showImg(HttpServletRequest request, HttpServletResponse response, String url) {
		String fileExtensionName = url.substring(url.lastIndexOf(".")+1);//jpg等
		String pattern = "(bmp|jpg|png|tif|gif|pcx|tga|exif|fpx|svg|psd|cdr|pcd|dxf|ufo|eps|ai|raw|WMF|webp)";
		if (!Pattern.matches(pattern, fileExtensionName)) {
			return ServerResponse.createByErrorMsg("读取失败");
		}
		...
		return ServerResponse.createBySuccessMsg("读取成功");
	}
