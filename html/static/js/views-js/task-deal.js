$(function () {
    get_task_of_user_now(1);
});

//请求get_task_of_user_now接口数据
function get_task_of_user_now(pageNum) {
...
function set_task_html(result) {
    ...
    $.each(taskList,function (index, item) {
        ...
        taskTitleTd.click(function () {
            window.location.href="task_deal_details?id="+item.taskId+"&html=task_deal";
        });
        var taskDetailsTd=$("<td class='content-wrap'></td>").append(item.taskDetails);
...
function set_pager(pageList) {
    ...
    if(pageList.hasPreviousPage==true){
        ...
        prePageLi.click(function () {
            get_task_of_user_now(pageList.prePage);
        });
    ...
    if(pageList.hasNextPage==true) {
        ...
        lastPageLi.click(function () {
            get_task_of_user_now(pageList.lastPage+1);
        });
        ...
        nextPageLi.click(function () {
            get_task_of_user_now(pageList.nextPage);
        });
...
$(document).on("click",".pagination-btn",function () {
    get_task_of_user_now($(this).text());
});
//点击跳转
$(document).on("click",".jump-pager-button",function () {
    get_task_of_user_now($(".jump-pager-input").val());
});
